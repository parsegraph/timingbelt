...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>timingbelt</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>timingbelt <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-timingbelt.js'>parsegraph-timingbelt.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See timingbelt build information at http://localhost:${port}`);
});
